import numpy as np
def  dVrdv0(xL, yL, hL, t, v0, alpha, k, x0, h0, m, g, deltaR):
    out = ((np.cos(alpha) * (x0 - xL + (m * np.log((k * t * v0 * np.cos(alpha)) / m + 1)) / k)) / (
        ...
        np.cos(t * ((g * k) / m) ** (1 / 2)) + v0 * np.sin(t * ((g * k) / m) ** (1 / 2)) * np.sin(alpha) * (k / (g * m)) ** (
                    1 / 2))) / k) * (g * k * m) ** (1 / 2)) / (
                       ...
                       k * t * v0 * np.cos(alpha) ** 2 * (x0 - xL + (m * np.log((k * t * v0 * np.cos(alpha)) / m + 1)) / k)) / (
                       m * ((k * t * v0 * np.cos(alpha)) / m + 1) ** 2) + (
                       m * np.sin(t * ((g * k) / m) ** (1 / 2)) * np.sin(alpha) * (
                           v0 * np.sin(alpha) * (g * k * m) ** (1 / 2) - g * m * np.tan(t * ((g * k) / m) ** (1 / 2))) * (
                                   k / (g * m)) ** (1 / 2)) / (
                       k * ((g * k * m) ** (1 / 2) + k * v0 * np.tan(t * ((g * k) / m) ** (1 / 2)) * np.sin(alpha)) * (
                           np.cos(t * ((g * k) / m) ** (1 / 2)) + v0 * np.sin(t * ((g * k) / m) ** (1 / 2)) * np.sin(alpha) * (
                               k / (g * m)) ** (1 / 2)))) / (
