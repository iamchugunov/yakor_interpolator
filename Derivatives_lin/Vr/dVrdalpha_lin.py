...
def dVrdalpha(xL, yL, hL, t, v0, alpha, k, x0, h0, m, g, deltaR):
    out = (v0 * np.exp(-(k * t) / m) * np.sin(alpha) * (
                xL - x0 + (m * v0 * np.cos(alpha) * (np.exp(-(k * t) / m) - 1)) / k) - v0 * np.exp(-(k * t) / m) * np.cos(alpha) * (
                       hL - h0 + (m * (v0 * np.sin(alpha) + (g * m) / k) * (np.exp(-(k * t) / m) - 1)) / k + (
                           g * m * t) / k) - (m * v0 * np.cos(alpha) * (
                v0 * np.exp(-(k * t) / m) * np.sin(alpha) + (g * m * (np.exp(-(k * t) / m) - 1)) / k) * (
                                                          np.exp(-(k * t) / m) - 1)) / k + (
                       m * v0 ** 2 * np.exp(-(k * t) / m) * np.cos(alpha) * np.sin(alpha) * (np.exp(-(k * t) / m) - 1)) / k) / (
