...
def d2rdkdv0_lin(xL, yL, hL, t, v0, alpha, k, x0, h0, m, g, deltaR):
    out = (((2 * m * np.cos(alpha) * (np.exp(-(k * t) / m) - 1) * (
                ...
                (m * np.cos(alpha) * (np.exp(-(k * t) / m) - 1)) / k ** 2 + (t * np.exp(-(k * t) / m) * np.cos(alpha)) / k) * (
                                                                                xL - x0 + (m * v0 * np.cos(alpha) * (
                                                                                    np.exp(-(k * t) / m) - 1)) / k) + 2 * (
                                                                                (m * np.sin(alpha) * (np.exp(
                                                                                    -(k * t) / m) - 1)) / k ** 2 + (
                                                                                            t * np.exp(-(k * t) / m) * np.sin(
                                                                                        alpha)) / k) * (hL - h0 + (
                m * (v0 * np.sin(alpha) + (g * m) / k) * (np.exp(-(k * t) / m) - 1)) / k + (g * m * t) / k) + (
                                                                                2 * m * np.cos(alpha) * ((t * v0 * np.exp(
                                                                            -(k * t) / m) * np.cos(alpha)) / k + (
                                                                                                                  m * v0 * np.cos(
                                                                                                              alpha) * (
                                                                                                                              np.exp(
                                                                                                                                  -(
                                                                                            ...
                                                                                            np.exp(-(
                                                                                                        k * t) / m) - 1)) / k + (
                                                                                2 * m * np.sin(alpha) * (
                                                                                    np.exp(-(k * t) / m) - 1) * ((t * np.exp(
                                                                            -(k * t) / m) * (v0 * np.sin(alpha) + (
                                                                                    g * m) / k)) / k + (g * m ** 2 * (
                                                                                    np.exp(-(k * t) / m) - 1)) / k ** 3 + (
                                                                                                                          m * (
                                                                                                                              v0 * np.sin(
